...
/** Abas visíveis conforme a seção do painel. */
type WhatsAppView = "ia" | "atendimento";
type TabKey = "conexao" | "configuracoes" | "conversas";

const VIEW_TABS: Record<WhatsAppView, TabKey[]> = {
  // "Configuração da IA": conexão do WhatsApp + configuração da IA
  ia: ["conexao", "configuracoes"],
  // "Atendimento": só as conversas ao vivo
...
export default function WhatsAppIaClient({
  ...
  return (
    ...
    >
      ...
      {/* Abas — só as da seção atual; escondidas quando há uma só */}
          ...
          <div className="flex shrink-0 flex-wrap items-center justify-between gap-3 rounded-2xl border border-stone-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <span className="text-sm font-semibold text-stone-800 dark:text-slate-100">
                  Atendimento da IA
                </span>
                <span
                ...
                >
                  {globalPause.paused
                    ? `Pausado (${formatUntil(globalPause.until)})`
                    : "Ativo"}
                </span>
              </div>
              <p className="mt-0.5 text-xs text-stone-500 dark:text-slate-400">
                {globalPause.paused
                  ? "A IA não está respondendo nenhum cliente."
                  : "A IA responde os clientes automaticamente."}
              </p>
            </div>
            ...
            {globalPause.paused ? (
              ...
              <div className="relative shrink-0">
                <button
                  onClick={() => setGlobalPauseMenu((v) => !v)}
                  disabled={pauseBusy}
                  className="inline-flex items-center gap-1 rounded-lg border border-amber-300 px-4 py-2 text-sm font-semibold text-amber-700 transition hover:bg-amber-50 disabled:opacity-60 dark:border-amber-700 dark:text-amber-300 dark:hover:bg-amber-950/40"
                >
                  Pausar tudo
                ...
                {globalPauseMenu && (
                  <>
                    <div
                      ...
                      onClick={() => setGlobalPauseMenu(false)}
                    />
                    <div className="absolute right-0 top-full z-30 mt-2 w-52 rounded-2xl border border-stone-200 bg-white p-2 shadow-xl dark:border-slate-700 dark:bg-slate-900">
                      ...
                      {PAUSE_DURATIONS.map((d) => (
                        <button
                          key={d.label}
                          onClick={() => {
                            pauseGlobal(d.minutes);
                            setGlobalPauseMenu(false);
                          }}
